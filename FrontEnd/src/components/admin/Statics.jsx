...
export default function Statics() {
  const { data: itemData, loading, error } = useGet('http://localhost:8080/statics/item');

  if (loading) return <div>Loading...</div>;
  if (error) return <div>Error: {error}</div>;
  if (!itemData) return <div>No data</div>;

  const filteredData = itemData.filter(item => item.total_sold > 0 || item.total_income > 0);
